...
== AJAX Example:
	...
	You cannot use the tinymce textarea fomr helper when using this initializing mehtodm but you can
	add your own classname for your textarea and pass this as paramter to the init_tinymce helper.
	
=== Example viewfile:
	
	<% form_for @post do |f| %>
	<%= f.text_area :body, class => "tinymce_ajax" %>
	<% end %>
	<%= init_tinymce('tinymce_ajax') %>
